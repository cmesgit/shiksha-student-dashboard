...
const Ic = {
  BookOpen:      () => <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>,
...
export default function SkillDevStudentSection({ data }) {
  ...
  const stats = [
    { value: enrolledCount,        label: "Enrolled courses", Icon: Ic.BookOpen    },
    { value: lessonsDone,          label: "Lessons done",     Icon: Ic.CheckCircle },
    { value: `${hoursLearned}h`,   label: "Hours learned",    Icon: Ic.Clock       },
    { value: upcomingCount,        label: "Upcoming",         Icon: Ic.Calendar    },
  ];
        ...
        {courses.length === 0 ? (
          ...
          <div key={c.id ?? i} style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 14, padding: "15px 18px", marginBottom: 12, boxShadow: "0 2px 10px rgba(18,80,39,.04)" }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
              {/* Icon */}
              <div style={{ width: 44, height: 44, borderRadius: 12, background: C.cream2, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: C.orange }}>
                <Ic.Zap />
              </div>
        ...
        {experts.length === 0
          ...
          : experts.map((e, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, background: "#fff", border: `1px solid ${C.border}`, borderRadius: 12, padding: "10px 12px", marginBottom: 8 }}>
              ...
              <button onClick={() => navigate(`/skill-dev/messages/${e.id}`)} style={{ all: "unset", cursor: "pointer", width: 30, height: 30, borderRadius: 8, background: C.cream2, display: "flex", alignItems: "center", justifyContent: "center", color: C.forest }}>
                <Ic.MessageSquare />
              </button>
            </div>
          ))
